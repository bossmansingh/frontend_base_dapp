...
contract NFTContract is ERC721, BaseContract {
    ...
    modifier validRequest(uint winTime) {
      uint counterValue = currentCounterValue();
      // Check and verify if the sent value is equal to current minting fee
      require(msg.value == _mintingFee, "Passed token value should be equal to current minting fee.");
      ...
      // Check and verify the current `_tokenCounter` does not belong to any existing NFT
      require(ownerOf(counterValue) == Helpers.nullAddress(), "The token already has a owner");
      // Check and verify if the current `_tokens` array does not contain a token at `counterValue`
      require(_tokens[counterValue].id == 0, "The token already exists");
    ...
    function createAndMintToken(
      ...
      uint8 killScore,
      bool isRare
    ) external payable nonReentrant onlyOwner() validRequest(winTime)  {
    ...
    function _createAndMintToken(
      ...
      _safeMint(msg.sender, counterValue); // TODO: Add if caller is winnerAddress in GameContract
     ...
     *
     * @param counterValue current value of counter
     * @param winTime total time taked by winner to win this game.
     * @param killScore count of longest consecutive kill by one single player on board
     * @param isRare true if this token is Rare, false otherwise
     */
